          ...
          <div className="relative md:hidden shrink-0" ref={menuRef}>
            ...
            {isMobileMenuOpen && (
              ...
              >
                ...
                <button
                  type="button"
                  id="btn-nav-reset-users-data"
                  onClick={() => {
                    setIsMobileMenuOpen(false);
                    if (window.confirm('¿Deseas cerrar todas las sesiones, borrar los usuarios locales y comenzar desde cero?')) {
                      onResetSeedData();
                    }
                  }}
                  className="w-full px-3 py-2 text-left text-[11px] text-rose-600 hover:text-rose-700 hover:bg-rose-50 flex items-center gap-2.5 transition-colors cursor-pointer border-t border-slate-100 mt-1"
                  title="Cierra todas las sesiones, borra usuarios y regresa a cero"
                >
                  <div className="w-6 h-6 rounded-lg bg-rose-100 text-rose-600 flex items-center justify-center shrink-0">
                    <Trash2 className="w-3.5 h-3.5" />
                  </div>
                  <span className="font-semibold">Borrar usuarios y reiniciar</span>
                </button>
              </div>
            )}
          </div>
